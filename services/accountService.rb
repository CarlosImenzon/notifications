...
class AccountService
  ...
  def self.change_pass(password1, password2, currentuser)
    if password1 != password2
      ...
      raise ValidationModelError.new('Error al cambiar contraseña
      o ingresaste la misma contraseña', 1)
    end
  end
  ...
  def self.change_mail(email1, email2, currentuser)
    if email1 != email2
      ...
      raise ValidationModelError.new('Error al cambiar email
       o ingresaste el mismo email', 1)
    end
  end
end
